...
def check_solution(
    ...
    """
    Checks feasibility aligned with your AMPL model logic.

    Constraints checked:
      - Each customer assigned to exactly one store (given by assign dict)
      - Inventory limits per store/product
      - Each node in N visited exactly once across all vehicles
      ...
      - Capacity: per vehicle, sum D[node] for visited nodes <= Q (in your data, only stores have D>0)
    ...
    # 3) Routing: routes exist for each vehicle and start/end at depot
    for k in K:
        if k not in routes:
            return False, f"Missing route for vehicle {k}."
        r = routes[k]
        if len(r) < 2:
            return False, f"Route for vehicle {k} is too short: {r}"
        if r[0] != depot or r[-1] != depot:
            return False, f"Route for vehicle {k} must start/end at depot: {r}"

    # 4) Each node in N visited exactly once across all vehicles (excluding depot)
    ...
    # For each vehicle, build node positions.
    for k in K:
        r = routes[k]
    ...
    # 6) Capacity per vehicle: sum D[node] over visited nodes <= Q
    for k in K:
        load_sum = sum(instance.D.get(node, 0.0) for node in routes[k] if node != depot)
